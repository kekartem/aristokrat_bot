...
async def on_foundation(message: types.Message, state: FSMContext):
    if message.text == 'Что это означает?':
        ...
        await message.answer('Хорошо. Имейте в виду, что перед установкой барбекю-зоны необходимо чтобы покрытие выдерживало ее вес. Средний вес барбекю- зон из кирпича - от 1000 до 4000 кг. Мы, как производитель, советуем заливать бетонную плиту, но и свайный фундамент также хорошо подойдет.')
